...
sys.path.append('/home/pjanowsk/c/Phenix/boostplay3')
import phenix_amber_interface as PAI
#~ import amber_adaptbx_ext.so 
#~ import boost.python
...
class geometry_manager(object):
  ...
  def energies_sites(self):
    ...
    sites_cart_c=PAI.ExtractVec(self.sites_cart.as_double())
    gradients_c=PAI.ExtractVec(self.gradients)
    target_c=PAI.ExtractVec(self.residual_sum)
    
    # Call c++ interface to call mdgx to calculate new gradients and target
    PAI.callMdgx(sites_cart_c, gradients_c, target_c, self.prmtop, self.ambcrd)
