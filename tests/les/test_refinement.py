...
from amber_adaptbx.tests.config import (
    PDB_COLLECTION, PDB_MTZ_COLLECTION, MTZ_COLLECTION, saved_2igd_prmtop_file,
    saved_2igd_rst7_file, saved_2igd_pdb_file, saved_2igd_mtz_file)


@pytest.mark.slow
@pytest.mark.parametrize(
    'prmtop_file, rst7_file, pdb_file, mtz_file, use_amber', [
        (saved_2igd_prmtop_file, saved_2igd_rst7_file, saved_2igd_pdb_file,
         saved_2igd_mtz_file, True),
        (saved_2igd_prmtop_file, saved_2igd_rst7_file, saved_2igd_pdb_file,
         saved_2igd_mtz_file, False),
    ])
...
@pytest.mark.slow
@pytest.mark.parametrize('pdb_file, mtz_file', PDB_MTZ_COLLECTION)
def test_LES_refinement_after_running_AmberPrep_use_amber_True_and_do_minimization_with_amber_h(
        pdb_file, mtz_file):
    """ just ensure no error. There is no assertion here """
    LES = True
    command_build = [
