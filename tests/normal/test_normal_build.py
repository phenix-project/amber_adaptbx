...
def test_geometry_minimization(pdb_file):
  ...
  with tempfolder():
    prmtop_file, rst7_file, new_pdb = get_prmtop_and_rst7_and_pdb_filenames_from_pdb(pdb_file)
...
def test_refinement_phenix(pdb_file, mtz_file):
  ...
  command_build = ['phenix.AmberPrep',
          '{pdb_file}'.format(pdb_file=pdb_file)
  ]
  command_refine = ['phenix.refine',
